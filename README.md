...
- **B站官方动态推送**：自动轮询官方 B站账号最新动态，基于 WBI 混淆签名、访客池与富文本清洗，渲染高保真动态长图卡片（包含自适应九宫格多图网格、抽奖徽章、话题标签、视频封面等）主动推送入群。
- **活动到期提醒**：在活动结束前（默认提前 48 小时）自动扫描并在群聊中发送提醒，避免遗漏奖励。
- **多主题主动推送管理**：支持群管理员通过 `/push` 指令独立开启或关闭不同功能的主动推送（`expiry` 到期提醒、`poster` 版本日历海报、`bili` B站动态），各主题互不干扰。
- **高并发单飞与单槽位缓存**：渲染层配备 **Singleflight（单飞并发合并）** 与 **Single-Slot（单图槽位缓存）**，无论多少个群并发接收推送，底层无头浏览器严格只画一次，旧图自动回收，长期运行绝对零内存泄漏。
