...
- **时间标记只剩一条**：红色竖线钉在出图时刻（分钟级，三轨各画一段、带圆点头；窗口不含该时刻就整体不画）。
  原先的"今天整列黄底"（整列近似，比条带粗糙）与"玩法期止红双线"（与每条带的右端重复，且三页签语义不一致：
  上一版本贴右缘、最新版本没有下一次维护数据）都删了。
...
| 时间表文字溢出 | `.probe/calpng/fit.js`（真排版量字宽 + 图例色块是否压字，负对照 = 调小 PPD） | 本机 node + Chrome |
