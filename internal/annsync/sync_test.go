...
func TestRetentionWindowFiltersAncientAnnouncements(t *testing.T) {
	...
	// 四篇公告：近期 (10天前)、临界 (94天前)、远古 (97天前)、零值时间
	rRecent := ref("recent")
	...
	src.setItemWithPub("ancient", "远古活动", base.Add(-97*24*time.Hour),
		ev("远古活动", base.Add(-97*24*time.Hour), base.Add(-30*24*time.Hour), annsync.StatusOK))
	src.setItemWithPub("boundary", "临界活动", base.Add(-94*24*time.Hour),
		ev("临界活动", base.Add(-94*24*time.Hour), base.Add(5*24*time.Hour), annsync.StatusOK))
